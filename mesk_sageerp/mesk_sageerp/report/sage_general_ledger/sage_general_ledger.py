...
from frappe.utils import cstr, getdate, format_date
...
def execute(filters=None):
	...
	# Count rows per voucher_no and assign to number_of_distributions
	
	voucher_counts = Counter(item.get("voucher_no") for item in filtered_accounts if item.get("voucher_no"))
	for item in filtered_accounts:
		item["number_of_distributions"] = voucher_counts.get(item.get("voucher_no"), 0)

	return columns, filtered_accounts
...
def get_result_as_list(data, filters):
	...
	for d in data:
		...
		if d.get("voucher_type") and (d.get("credit", 0) > 0 or d.get("voucher_type") != "Purchase Invoice"):
			continue
...
def get_columns(filters):
	...
	columns = [
		...
		{
			"label": _("Accounts Payable Account"),
			"fieldname": "accounts_payable_account",
			"fieldtype": "Data",
			"width": 130,
		},
		{
			"label": _("Number of Distributions"),
			"fieldname": "number_of_distributions",
			"fieldtype": "Data",
			"width": 100,
		},
		{

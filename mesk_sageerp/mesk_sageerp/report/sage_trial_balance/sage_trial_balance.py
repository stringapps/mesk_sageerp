...
from frappe.utils import add_days, cstr, flt, formatdate, getdate ,format_date
...
def execute(filters=None):
	...
	filtered_accounts = [
		{	**item, 
			...
			"invoice_cm": str(srs.invoice_prefix)+'-'+str(format_date(filters.from_date)).replace("-", ""),
			"credit_memo": srs.get("credit_memo"),
			"today_date":format_date(filters.from_date),
...
def get_columns():
	return [
		...
		{
			"fieldname": "accounts_receivable_account",
			"fieldtype": "Data",
			"label": "Accounts Receivable Account",
			"width": 120
		},
		{
			"fieldname": "accounts_receivable_amount",
			"fieldtype": "Data",
